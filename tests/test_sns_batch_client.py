...
async def test_publish(topic, queue):
    ...
    batch_client = SNSBatchClient(sns_client=client, max_attempts=4, topic_arn=topic.arn)
    invalid_entries = [
        {
            "Id": str(uuid.uuid4()),
            "Message": "message",
    ...
    response = await batch_client.publish_messages(itertools.chain(*zip(invalid_entries, entries)))
...
class PublishBatchMock:
    ...
    def publish_batch(self, _TopicArn, PublishBatchRequestEntries):
        self.calls += 1
...
async def test_publish_on_retryable_errors():
    max_attempts = 4
    client = Mock(publish_batch=Mock())
    client.publish_batch.side_effect = PublishBatchMock(max_attempts).publish_batch
    batch_client = SNSBatchClient(sns_client=client, max_attempts=max_attempts, topic_arn="test")
    entries = [
        {
            "Id": str(uuid.uuid4()),
            "Message": f"message{i}",
    ...
    response = await batch_client.publish_messages(entries)
    assert len(response["Successful"]) == len(entries)
