...
First, create an SNS client and an SNS Batch Client for some topic:
```python
import boto3
from sns_batch_client import SNSBatchClient
client = boto3.client("sns")
batch_client = SNSBatchClient(sns_client=client, max_attempts=4, topic_arn="topic")
```
...
coro = await batch_client.publish_messages(entries)
